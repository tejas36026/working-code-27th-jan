...
let appleLogoImageData = null; // This will hold the Apple logo ImageData
...
self.onmessage = function(e) {
  ...
  try {
    ...
    const width = appleLogoImageData.width;
    const height = appleLogoImageData.height;
    ...
    if (!canvas) {
      ...
      // Store Apple logo as original image
      appleLogoImageData = new ImageData(
    ...
    // Draw scaled Apple logo
    ctx.save();
    ctx.translate(width / 2, height / 2);
    ctx.scale(backgroundScale, backgroundScale);
    ctx.translate(-width / 2, -height / 2);
    ctx.drawImage(appleLogoImageData, 0, 0);
    ctx.restore();
